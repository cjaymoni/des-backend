...
export class ManifestJobService {
  ...
  async findAll(
    ...
    return this.tenantService.withManager(async (manager) => {
      ...
      if (search.jobNo) qb.andWhere('mj.jobNo ILIKE :jobNo', { jobNo: `%${search.jobNo}%` });
      if (search.hblNo) qb.andWhere('mj.hblNo = :hblNo', { hblNo: search.hblNo });
      if (search.consigneeDetails) qb.andWhere('mj.consigneeDetails ILIKE :c', { c: `%${search.consigneeDetails}%` });
      if (search.custRefNo) qb.andWhere('mj.custRefNo = :custRefNo', { custRefNo: search.custRefNo });
      if (search.houseManifestId) qb.andWhere('mj.houseManifestId = :houseManifestId', { houseManifestId: search.houseManifestId });
      if (search.paidStatus !== undefined) qb.andWhere('mj.paidStatus = :paidStatus', { paidStatus: search.paidStatus });
  ...
  async findOne(id: string): Promise<ManifestJob> {
    return this.tenantService.withManager((manager) => this.fetchOne(manager, id));
  }

  async create(data: CreateManifestJobDto, userId: string): Promise<ManifestJob> {
    return this.tenantService.withManager(async (manager) => {
      const existing = await manager.getRepository(ManifestJob).findOne({ where: { jobNo: data.jobNo } });
      if (existing) throw new ConflictException(`Job number "${data.jobNo}" already exists`);

      const house = await manager.getRepository(HouseManifest).findOne({ where: { id: data.houseManifestId } });
  ...
  async update(id: string, data: UpdateManifestJobDto, userId: string): Promise<ManifestJob> {
    return this.tenantService.withManager(async (manager) => {
      const existing = await manager.getRepository(ManifestJob).findOne({ where: { id } });
      ...
      if (data.releaseStatus !== undefined || data.releaseDate !== undefined) {
        ...
        if (data.releaseStatus !== undefined) houseUpdate.releaseStatus = data.releaseStatus;
        if (data.releaseDate !== undefined) houseUpdate.releaseDate = data.releaseDate;
        await manager.getRepository(HouseManifest).update(existing.houseManifestId, houseUpdate);
      }
