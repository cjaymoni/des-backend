...
export class TransactionPurposeService {
  ...
  findAll(): Promise<TransactionPurpose[]> {
    return this.tenantService.withManager(
      (m) =>
        m
          .getRepository(TransactionPurpose)
          .find({ order: { purposeCode: 'ASC' } }),
      { transactional: false },
    );
  }

  create(
    data: TransactionPurposeDto,
    userId: string,
  ): Promise<TransactionPurpose> {
    return this.tenantService.withManager(async (m) => {
      const existing = await m
        .getRepository(TransactionPurpose)
        .findOne({ where: { purposeCode: data.purposeCode } });
      if (existing)
        throw new BadRequestException(
          `Purpose code "${data.purposeCode}" already exists`,
        );
      return m.save(
        m.create(TransactionPurpose, { ...data, createdBy: userId }),
      );
    });
  }
  ...
  update(
    id: string,
    data: Partial<TransactionPurposeDto>,
    userId: string,
  ): Promise<TransactionPurpose> {
    return this.tenantService.withManager(async (m) => {
      const existing = await m
        ...
        .findOne({ where: { id } });
      if (!existing)
        throw new NotFoundException('Transaction purpose not found');
      await m
        .getRepository(TransactionPurpose)
        .update(id, { ...data, updatedBy: userId });
      return m
        .getRepository(TransactionPurpose)
        .findOne({ where: { id } }) as Promise<TransactionPurpose>;
    });
  }
...
export class TransactionPurposeDetailService {
  ...
  create(
    data: TransactionPurposeDetailDto,
    userId: string,
  ): Promise<TransactionPurposeDetail> {
    return this.tenantService.withManager(async (m) => {
      const purpose = await m
        .getRepository(TransactionPurpose)
        .findOne({ where: { purposeCode: data.purposeCode } });
      if (!purpose)
        throw new NotFoundException(
          `Purpose code "${data.purposeCode}" not found`,
        );
      return m.save(
        m.create(TransactionPurposeDetail, { ...data, createdBy: userId }),
      );
    });
  }
  ...
  update(
    id: string,
    data: Partial<TransactionPurposeDetailDto>,
    userId: string,
  ): Promise<TransactionPurposeDetail> {
    return this.tenantService.withManager(async (m) => {
      const existing = await m
        .getRepository(TransactionPurposeDetail)
        .findOne({ where: { id } });
      ...
      return m
        .getRepository(TransactionPurposeDetail)
        .findOne({ where: { id } }) as Promise<TransactionPurposeDetail>;
    });
  }
