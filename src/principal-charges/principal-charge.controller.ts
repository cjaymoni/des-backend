...
export class PrincipalChargeController {
  ...
  @Get('audit-log')
  getAuditLog(): Promise<ChargeSetupAuditLog[]> {
  ...
  @Get(':principalId/audit-log')
  getAuditLogByPrincipal(
    @Param('principalId') principalId: string,
  ): Promise<ChargeSetupAuditLog[]> {
    return this.service.getAuditLogByPrincipal(principalId);
  }

  @Post()
  upsert(
    @Body() data: UpsertPrincipalChargeSetupDto,
  ...
  delete(
    ...
    return this.service.delete(principalId, req.user.id);
  }
}
