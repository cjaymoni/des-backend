...
export class IncomeExpenditureService {
  ...
  async findAll(
    ...
    return this.tenantService.withManager(
      async (manager) => {
        ...
        if (search.transRemarks)
          qb.andWhere('ie.transRemarks = :transRemarks', {
            transRemarks: search.transRemarks,
          });
        if (search.transType)
          qb.andWhere('ie.transType = :transType', {
            transType: search.transType,
          });
        if (search.consignee)
          qb.andWhere('ie.consignee ILIKE :consignee', {
            consignee: `%${search.consignee}%`,
          });
        if (search.hbl) qb.andWhere('ie.hbl = :hbl', { hbl: search.hbl });
        if (search.strMonth)
          qb.andWhere('ie.strMonth = :strMonth', { strMonth: search.strMonth });
        if (search.dateYear)
          qb.andWhere('ie.dateYear = :dateYear', { dateYear: search.dateYear });
    ...
    return this.tenantService.withManager(
      async (manager) => {
        const record = await manager
          ...
          .findOne({ where: { id } });
        if (!record)
          throw new NotFoundException('Income/Expenditure record not found');
        return record;
      },
      { transactional: false },
    );
  ...
  async create(
    data: CreateIncomeExpenditureDto,
    userId: string,
  ): Promise<IncomeExpenditure> {
    return this.tenantService.withManager(async (manager) => {
      const record = manager.create(IncomeExpenditure, {
        ...data,
        createdBy: userId,
      });
  ...
  async update(
    id: string,
    data: UpdateIncomeExpenditureDto,
    userId: string,
  ): Promise<IncomeExpenditure> {
    return this.tenantService.withManager(async (manager) => {
      ...
      return manager
        .getRepository(IncomeExpenditure)
        .findOne({ where: { id } }) as Promise<IncomeExpenditure>;
    });
  }
