...
export class CifValueService {
  ...
  async getSettings(): Promise<CifSettings | null> {
    return this.tenantService.withManager(
      (manager) => manager.getRepository(CifSettings).findOne({ where: {} }),
      { transactional: false },
    );
  }

  async upsertSettings(
    data: UpsertCifSettingsDto,
    userId: string,
  ): Promise<CifSettings> {
  ...
  async findByRefNo(refNo: string): Promise<CifValue[]> {
    return this.tenantService.withManager(
      (manager) =>
        manager
          .getRepository(CifValue)
          .find({ where: { refNo }, order: { createdAt: 'ASC' } }),
      { transactional: false },
    );
  }
  ...
  async update(
    ...
    return this.tenantService.withManager(async (manager) => {
      ...
      return manager
        .getRepository(CifValue)
        .findOne({ where: { id } }) as Promise<CifValue>;
    });
  }
