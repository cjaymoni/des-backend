...
export class PrincipalService {
  ...
  async findAll(
    ...
    return this.tenantService.withManager(
      async (manager) => {
        ...
        if (search.name)
          qb.andWhere('p.name ILIKE :name', { name: `%${search.name}%` });
        if (search.isActive !== undefined)
          qb.andWhere('p.isActive = :isActive', { isActive: search.isActive });
  ...
  async create(data: CreatePrincipalDto, userId: string): Promise<Principal> {
    return this.tenantService.withManager(async (manager) => {
      ...
      if (existing)
        throw new ConflictException(`Principal "${data.name}" already exists`);

      const p = manager.create(Principal, {
        ...data,
      ...
      try {
        ...
        if (err.code === '23505')
          throw new ConflictException(
            `Principal "${data.name}" already exists`,
          );
        throw err;
      }
    });
  ...
  async update(
    ...
    return this.tenantService.withManager(async (manager) => {
      ...
      return manager
        .getRepository(Principal)
        .findOne({ where: { id } }) as Promise<Principal>;
    });
  }
